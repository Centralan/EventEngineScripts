...
local grave6sound = Location:new(world, 91.0, 74.0, -614.0);
...
local tp3 = Location:new(world, 121.0, 154.0, -372.0);
...
local candy11 = Location:new(world, -68.0, 65.0, -515.0);
...
function welcome2(data)
	...
	spawnsound:playSound('WOLF_HOWL', 3, 0.8);
end
...
local grave6ChestPlayers = {};
local grave6ChestResetTimer = Timer:new("grave6_reset_chest", 20 * 60 * 5);
local grave6ChestResetTimerRunning = false;
...
function ach_top(data)
         local player = Player:new(data.player);
         player:sendEvent("achievement.topexcavator");
end

registerHook("INTERACT", "ach_top", 143, "spawn2", -72, 65, -504);
...
function knock1(data)
	...
	knock1sound:playSound('ZOMBIE_WOOD', 1, 5);
	knock1sound:playSound('ZOMBIE_WOOD', 1, 5);
	knock2sound:playSound('ZOMBIE_WOOD', 1, 5);
end
...
function dracula1(data)
	...
	tree1sound:playSound('VILLAGER_HAGGLE', 5, 0.5);
	else
...
function pit1(data)
	...
	tree1sound:playSound('LAVA', 10, 0.2);
	player:teleport(tp1);
end
end
...
function piston_extend(data)
        local player = Player:new(data.player);
        cave1sound:playSound('PISTON_EXTEND', 5, 0.5);
end

function piston_retract(data)
        local player = Player:new(data.player);
        cave1sound:playSound('PISTON_RETRACT', 5, 0.5);
end

function enderdragon_growl(data)
        local player = Player:new(data.player);
       cave1sound:playSound('ENDERDRAGON_GROWL', 5, 0.5);
end

function horse_skeleton_death(data)
        local player = Player:new(data.player);
        cave1sound:playSound('HORSE_SKELETON_DEATH', 5, 0.5);
end

function horse_skeleton_idle(data)
        local player = Player:new(data.player);
        cave1sound:playSound('HORSE_SKELETON_IDLE', 5, 0.5);
end
...
function house_enderman(data)
        local player = Player:new(data.player);
	spencer1sound:playSound('ENDERMAN_SCREAM', 1, 0.5);
        EventEngine.player.addPotionEffect(player.name, 'BLINDNESS', 10, 5);
end
...
function house_zombie(data)
        local player = Player:new(data.player);
	spencer1sound:playSound('HORSE_ZOMBIE_DEATH', 1, 0.7);
        EventEngine.player.addPotionEffect(player.name, 'BLINDNESS', 10, 5);
end
...
function cave_blind_remove(data)
        local player = Player:new(data.player);
        player:sendMessage("&7You can see the moonlight ahead.");
...
registerHook("REGION_ENTER", "villager_enter2", "spawn2-h2023_cave_message");
...
function maze_reset1(data)
        local player = Player:new(data.player);
        player:sendMessage("(&6???&f) That chest was empty try again.");
	mazesound:playSound('ANVIL_LAND', 5, 0.5);
...
function tower_door(data)
	...
	tower1:playSound('DOOR_OPEN', 3, 0.5);
end

registerHook("INTERACT", "tower_door", 143, "spawn2", 107, 81, -476);
...
function tower_redstone(data)
	...
	if not keychestChestPlayers[player.name] then
		...
		tower1:playSound('WOOD_CLICK', 15, 0.5);
...
registerHook("INTERACT", "tower_redstone", 145, "spawn2", 110, 108, -472);
